...
$(function () {
    ...
    //点击右边小点显示导
    $(".muen-icon-wrap").on("click", function () {
        if ($(this).hasClass("open")) {
            $(".nav-header").css("border-bottom", "1px solid #5e9dd7");
            $(".nav-header .dropdown").css("border-bottom", "1px solid #4189c9");
            $(".navbar-static-top").animate({
                height: "120px"
            }, 300)
            $(".nav-left,.nav-right").show();
            ...
            $(".nav-header .dropdown,.nav-header").css("border-bottom", "none")
    ...
    //屏幕小于768，点击顶部导航展示侧边栏
    $("body").on("click", ".navbar-static-top .nav-left li", function () {
        var bodyW = $(window).width();
        //console.log($(".navbar-static-side").css("left"))
        if (bodyW < 768) {
            $("body").removeClass("mini-navbar");
            $(this).children("img").show().siblings("i").hide();
    ...
    $(".navbar-minimalize").on("click", function () {
        ...
        if ($("body").hasClass("mini-navbar")) {
            $(this).children("img").hide().siblings("i").show();
            $(".nav-second-level").removeAttr("style");
            //$(this).children("i").removeClass("fa-long-arrow-left ").addClass("fa-bars");
        } else {
            $(this).children("img").show().siblings("i").hide();

            //$(this).children("i").removeClass("fa-bars").addClass("fa-long-arrow-left ");
        }

        var bw = $("body").width();
        var cl = $("#content-main").offset().left;
        if ($("body").hasClass("mini-navbar")) {
            $("#content-main").css("width", bw - 70);
            $("#page-wrapper").css("width", bw - 70);
        } else {
            $("#content-main").css("width", bw - 220);
            $("#page-wrapper").css("width", bw - 220);
        }
    })

    function miniNavBar() {   //body添加min-navbar
        var bodyW = $(window).width();
        if (bodyW > 1200) {
            $("body").removeClass("mini-navbar");
            $("#content-main").css("width", bodyW - 220);
            $("#page-wrapper").css("width", bodyW - 220);
            $(this).children("img").hide().siblings("i").show();
            //$(".minimalize-styl-2 i").removeClass("fa-bars ").addClass("fa-long-arrow-left ");
        } else {
            $("body").addClass("mini-navbar");
            $("#content-main").css("width", bodyW - 70);
            $("#page-wrapper").css("width", bodyW - 70);
            $(this).children("img").show().siblings("i").hide();
            //$(".minimalize-styl-3 i").removeClass("fa-long-arrow-left ").addClass("fa-bars");
