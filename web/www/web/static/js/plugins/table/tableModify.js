...
(function() {
    ...
    var Metheds = {
        openIfram: function() {
            ...
            layer.open({
                ...
                yes: function(index) {
                    ...
                    Cookies.set(`${qs.ct}_${qs.ac}`, JSON.stringify(itemArray));
        ...
        getDate: function() {
            var data = [];
            if(!window.Cookies){
                return alert('tableModify.js 这需要依赖引入 static/frame/js/js.cookie.min.js');
            }
            let cacheArray = Cookies.get(`${qs.ct}_${qs.ac}`) ? JSON.parse(Cookies.get(`${qs.ct}_${qs.ac}`)) : [];
            $(".table").find("thead th").each(function(i) {
                ...
                const isCache = cacheArray.length === 0 ? true : cacheArray.find(item => item.name === name);
                Metheds.objToArr(data, name, i, fixed, !isCache ? 'no' : show);
            })
    ...
    /**
     * 新增 table-sticky 控制表格的方法
     */
    let cacheWidth = 0;
    $('.table-sticky thead th').each(function(index){
        if($(this).attr('data-style') === 'sticky') {
            if($(this).attr("width")){
                const width = $(this).attr("width") || 200;
                const left = $(this).prev().attr("width");

                if(left) {
                    cacheWidth = cacheWidth + Number(left);
                    $(this).attr("style", `width: ${width}px!important;left: ${cacheWidth}px;`);
                    $('.table-sticky tbody tr').each(function(){
                        $(this).find('td').eq(index).addClass('is-sticky').attr("style", `width: ${width}px!important;left: ${cacheWidth}px;`);
                    })
                }else{
                    $(this).attr("style", `width: ${width}px!important;`);
                    $('.table-sticky tbody tr').each(function(){
                        $(this).find('td').eq(index).addClass('is-sticky').attr("style", `width: ${width}px!important;`);
                    })
                }
            }
        }
