...
function pageFormSubmit() {
    ...
    if($('.page_size').val() == '-1'){
        // 克隆自定义输入框到selector后
        var tplsize = (Cookies && Cookies.get('table_page_size')) || 0;
        if(!$('#custom_size_val').length) {
            $('<input type="text" placeholder="'+tplsize+'" id="custom_size_val" value="">').insertAfter('.page_size');

            $('#custom_size_val').on('keyup', function(e){
                if(e.keyCode === 13){
                    pageFormSubmit();
                }
            })
        ...
        if($('#custom_size_val').val() === '') return;

        qs['page_size'] = $('#custom_size_val').val();
        Cookies && Cookies.set('table_page_size', $('#custom_size_val').val());
        return (location.href = '?' + $.param(qs));
    }

    Cookies && Cookies.set('table_page_size', $('.page_size').val());
...
$(function() {
    ...
    // 翻页插件自定义输入页数条转
    $('#custom_page_val').on('keyup', function(e){
        if(e.keyCode === 13){
            pageFormSubmit();
        }
    })
    ...
    layui.use(['form'], function () {
        ...
        form.on('switch(switchChange)',function (data) {
            ...
            const ID = data.elem.attributes['data-id'].nodeValue;
            const type = data.elem.attributes['data-type'].nodeValue;

            if(!ct) return alert('ct undefined');
            $.ajax({
                url: `/?ct=`+ct+`&ac=change&id=${ID}&type=${type}&state=${checked ? 1 : 0}`,
