...
class cls_filecache
{
    ...
    */ 
    ...
    public function delete( $key )
    {
        ...
        // 模仿redis返回值
        if ( isset($cur_node['data_len']) && $cur_node['data_len'] > 0 ) 
        {
            return 1;
        }
        else 
     ...
     * @return void
     */
    public function ttl( $key, $need_cur_node = false )
    {
        //检查文件是否已经打开
        $this->open();
        if ( !$key ) 
    ...
    */ 
     ...
     * @param $key
     * @param $value
     * @param $compress 是否压缩（无用选项，这里仅是为了和memcache方法一致）
     * @param $exptime 超时时间
     * @param $block_size (这版本弃用了此值)
...
// 测试
/////////////////////////
//$cfc = cls_filecache::factory( dirname(__FILE__).'/test' );
//for( $i = 0; $i < 10; $i++)
//{
    //$cfc->set("ttt_".$i, "v_".$i);
//}
