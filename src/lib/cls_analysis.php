...
// +----------------------------------------------------------------------
// | 1、必要函数 iconv
// | 2、本程序是使用RMM逆向匹配算法进行分词的，词库需要特别编译，本类里提供了make_dict()方法
// | 3、有简单的基础分词词库，如果分词词库与系统词汇相差比较远，可以自行修改 share/phpana_dict/not-build/base_dic_full.txt 里的词条，然后使用：
// |     cls_analysis::instance()->make_dict( $source_file, $target_file ); (文件名都用绝对路径),词条可在GitHub上获取，或自主定义规则，格式请参考base_dic_full.txt
// | 4、对主词典使用特殊格式进行编码, 不需要载入词典到内存操作
// | 5、编译后的词条存放在share/phpana_dict下
// | 备：如需要性能高的分词操作，建议使用swcs扩展进行分词，本类库只适用数据量小的操作
// +----------------------------------------------------------------------
// +----------------------------------------------------------------------
// | 简单用法
// | $str = "把一段短文本进行拆分";
...
// |        ->delimiter('-')
// |        ->exec();
// | delimiter()可指定分割符号，默认空格，如只需空格分隔可以不调用，如cls_analysis::instance()->set_source($str)->exec();
// | echo $ok_str;
// +----------------------------------------------------------------------
...
class cls_analysis
{
    ...
    public function __construct($source_charset = 'utf-8', $target_charset = 'utf-8', $source = '')
    {
        ...
        $this->set_source($source, $source_charset, $target_charset);
        if (self::$loadInit) $this->load_dict();
    }
    //单例
...
//        if (!self::$instance instanceOf  cls_analysis)
//        {
...
//        }
    ...
    function __destruct()
    {
        if ($this->main_dic_hand !== false) {
            @fclose($this->main_dic_hand);
        }
    }
     ...
     * @return short int
     */
    public function get_word_infos($key, $type = 'word')
    {
        if (!$this->main_dic_hand) {
            $this->main_dic_hand = fopen($this->mainDicFile, 'r');
        }
        $p = 0;
        ...
        if (isset($this->mainDicInfos[$keynum])) {
            ...
            //rewind( $this->main_dic_hand );
            $move_pos = $keynum * 8;
            fseek($this->main_dic_hand, $move_pos, SEEK_SET);
            $dat = fread($this->main_dic_hand, 8);
            $arr = unpack('I1s/n1l/n1c', $dat);
            if ($arr['l'] == 0) {
                return false;
            }
            fseek($this->main_dic_hand, $arr['s'], SEEK_SET);
            $data = @unserialize(fread($this->main_dic_hand, $arr['l']));
     ...
     * @return bool
     */
    public function set_source($source, $source_charset = 'utf-8', $target_charset = 'utf-8')
    {
     ...
     * @return void
     */
    public function set_result_type($rstype)
    {
     ...
     * @return void
     */
    public function load_dict($maindic = '')
    {
    ...
    {
        $winfos = $this->get_word_infos($word);
        return ($winfos !== false);
    }
    ...
    public function get_word_property($word)
    {
        ...
        $infos = $this->get_word_infos($word);
                        ...
                        while (true) {
                            ...
                            if ($w == $ew) {
                                ...
                                if (!isset($this->newWords[$tmpw])) {
                                    ...
                                    $this->set_word_infos($tmpw, array('c' => 1, 'm' => 'nb'));
                                }
                ...
                if ($slen < 5) {
                    ...
                    if ($lastType == 4 && (isset($this->addonDic['u'][$str]) || isset($this->addonDic['u'][substr($str, 0, 2)]))) {
                        ...
                        if (!isset($this->newWords[$this->simpleResult[$spos - 1]['w']])) {
                            ...
                            $this->set_word_infos($ww, array('c' => 1, 'm' => 'mu'));
                        }
    ...
    private function _deep_analysis_cn(&$str, $lastec, $spos, $slen, $optimize = true)
    {
        ...
        if ($spos > 0 && $slen < 11 && $this->simpleResult[$spos - 1]['w'] == $quote1) {
            ...
            if (!isset($this->newWords[$str])) {
                ...
                $this->set_word_infos($str, array('c' => 1, 'm' => 'nq'));
            }
            if (!$this->differMax) {
                $this->finallyResult[$spos][] = $str;
                return;
        ...
        if ($prePos > -1 && !isset($this->finallyResult[$prePos])) {
            ...
            if (($lastt == 4 || isset($this->addonDic['c'][$lastw])) && isset($this->addonDic['u'][$smarr[0]])) {
                ...
                if (!isset($this->newWords[$this->simpleResult[$prePos]['w']])) {
                    ...
                    $this->set_word_infos($this->simpleResult[$prePos]['w'], array('c' => 1, 'm' => 'mu'));
                }
                $smarr[0] = '';
                $i++;
            }
        ...
        for (; $i < $arlen; $i++) {
            ...
            if (isset($this->addonDic['c'][$cw]) && isset($this->addonDic['u'][$nw])) {
                ...
                if (!isset($this->newWords[$newarr[$j]])) {
                    ...
                    $this->set_word_infos($newarr[$j], array('c' => 1, 'm' => 'mu'));
                }
                $j++;
                $i++;
                $ischeck = true;
                ...
                if (!isset($this->addonDic['s'][$nw]) && strlen($nw) < 5 && !$is_rs) {
                    ...
                    if (!isset($this->newWords[$newarr[$j]])) {
                        $this->set_word_infos($newarr[$j], array('c' => 1, 'm' => 'nr'));
                ...
                if (!isset($this->addonDic['s'][$cw]) && !$is_rs) {
                    ...
                    if (!isset($this->newWords[$newarr[$j]])) {
                        ...
                        $this->set_word_infos($newarr[$j], array('c' => 1, 'm' => 'na'));
                    }
                    $i++;
                    $j++;
                    $ischeck = true;
                ...
                if (strlen($cw) == 2 && strlen($nw) == 2
                    ...
                    if (!isset($this->newWords[$newarr[$j]])) {
                        ...
                        $this->set_word_infos($newarr[$j], array('c' => 1, 'm' => 'ms'));
                    }
                    $i++;
                    $j++;
                    $ischeck = true;
     ...
     * 设置其他格式的符号
     */
    public function delimiter($spword = ' ')
    {
        if ($spword !== ' ')
        {
            $this->_spword = $spword;
     ...
     * @return string
     */
    public function exec($word_meanings = false)
    {
        $this->run();   //取得分析结果
        $rsstr = '';
        foreach ($this->finallyResult as $v) {
            ...
            $m = '';
            if ($word_meanings) {
                $m = $this->get_word_property($v['w']);
            }
            $w = $this->_out_string_encoding($v['w']);
            if ($w != ' ') {
                if ($word_meanings) {
                    $rsstr .= $this->_spword . $w . $m;
                } else {
                    $rsstr .= $this->_spword . $w;
                }
            }
        }
        return $rsstr;
     ...
     * @return array()
     */
    public function get_simple_result_all()
    {
        $rearr = array();
        foreach ($this->simpleResult as $k => $v) {
            $w = $this->_out_string_encoding($v['w']);
     ...
     */
    public function get_finally_index()
    {
        $rearr = array();
        foreach ($this->finallyResult as $v) {
            if ($this->resultType == 2 && ($v['t'] == 3 || $v['t'] == 5)) {
     ...
     * @return string
     */
    public function get_finally_keywords($num = 10)
    {
        $n = 0;
        $arr = $this->get_finally_index();
        $okstr = '';
        foreach ($arr as $k => $v) {
            //排除长度为1的词
            if (strlen($k) == 1) {
     ...
     * @return void
     */
    public function make_dict($source_file, $target_file = '')
    {
